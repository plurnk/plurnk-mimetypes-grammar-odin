#!/usr/bin/env node
// CI byte-identical reproducibility check for odin.wasm.
...
const committed = await readFile(path.join(repoRoot, "odin.wasm"));
const committedHash = createHash("sha256").update(committed).digest("hex");
console.log(`committed odin.wasm sha256: ${committedHash}`);

const work = await mkdtemp(path.join(tmpdir(), "grammar-odin-verify-"));
...
console.log(`rebuilt odin.wasm sha256: ${rebuiltHash}`);
