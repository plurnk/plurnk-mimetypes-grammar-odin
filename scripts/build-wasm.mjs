#!/usr/bin/env node
// Reproducible WASM build for tree-sitter-odin.
...
const work = await mkdtemp(path.join(tmpdir(), "grammar-odin-build-"));
await run("git", ["clone", "--no-checkout", "https://github.com/tree-sitter-grammars/tree-sitter-odin.git", "src"], { cwd: work });
...
// Locate produced wasm and copy as odin.wasm.
const fs = await import("node:fs/promises");
const built = (await fs.readdir(buildCwd)).find((f) => f.endsWith(".wasm"));
if (!built) throw new Error("no .wasm produced");
await copyFile(path.join(buildCwd, built), path.join(repoRoot, "odin.wasm"));
const bytes = (await readFile(path.join(repoRoot, "odin.wasm"))).length;
console.log(`odin.wasm: ${bytes} bytes (built from ${pin})`);
